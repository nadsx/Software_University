function breakfastRobot() {
    return (function () {
        ...
        function prepare(recipe, productQuantity) {
            const recipeIngredients = Object.entries(recipesObj[recipe]);

            for (const [ingredient, quantity] of recipeIngredients) {
                const neededIngredients = quantity * productQuantity;

                if (ingredientsObj[ingredient] < neededIngredients) {
                    return `Error: not enough ${ingredient} in stock`;
                }
            }

            for (const [ingredient, quantity] of recipeIngredients) {
                ingredientsObj[ingredient] -= quantity * productQuantity;
            }
